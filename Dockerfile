...
RUN cargo build --release --target=x86_64-unknown-linux-gnu  --bin algo-rust-bert-demo
...
#RUN apt-get update \
#    && apt-get install -y ca-certificates tzdata net-tools wget \
...
#
#ENV LIBTORCH=/opt/bitnami/python/lib/python3.8/site-packages/torch
#RUN export LIBTORCH=$(python3 -c 'import torch; from pathlib import Path; print(Path(torch.__file__).parent)')
#ENV DYLD_LIBRARY_PATH=${LIBTORCH}/lib
#ENV LD_LIBRARY_PATH=${LIBTORCH}/lib:$LD_LIBRARY_PATH
#
